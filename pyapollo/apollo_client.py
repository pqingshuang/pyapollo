...
class ApolloClient(object):
    ...
    def _get_clusters(self) -> dict:
        ...
        url = f"{self.host}:{self.port}/apps/{self.app_id}/envs/DEV/clusters"
    ...
    def _get_namespaces(self) -> dict:
        ...
        url = f"{self.host}:{self.port}/apps/{self.app_id}/envs/DEV/clusters/{self.cluster}/namespaces"
        r = self._http_get(url)
        if r.status_code == 200:
            # namespaces = r.json()
            return r.json()
            # return {_['baseInfo'].get("namespaceName"): _['baseInfo'].get("id") for _ in namespaces}
        else:
    ...
    def get_value(
        ...
        """
        try:
            print("`````````````", self._cache, "1324oqwe4trf")
    ...
    def _get_config_by_namespace(self, namespace: str = "application") -> None:
        ...
        url = f"{self.host}:{self.port}/apps/{self.app_id}/envs/DEV/clusters/{self.cluster}/namespaces/{namespace}/releases/latest"
        print(url)
        try:
            r = self._http_get(url)
            if r.status_code == 200:
    ...
    def _long_poll(self) -> None:
        try:
            ...
            for namespace in namespaces:
                _cache_k = namespace["baseInfo"]["namespaceName"]
                self._cache[_cache_k] = {}
                print(namespace["baseInfo"]["namespaceName"])
                print(self._cache)
                for item in namespace["items"]:
                    item = item["item"]
                    self._cache[_cache_k][item["key"]] = item["value"]
                self._update_local_cache(
                    str(time.time()),
                    self._cache,
                    _cache_k,
